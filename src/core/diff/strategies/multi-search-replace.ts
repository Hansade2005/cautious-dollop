...
function processLine(line: string): string {
	return line.replace(/\\<<<<<<< SEARCH|\\=======|\\>>>>>>> REPLACE/g, match => match.slice(1));
}

function processLineWithIndex(line: string, index: number): string {
	return processLine(line);
}

const lines = content.split('\n').map((line: string, i: number) => {
	return processLineWithIndex(line, i);
});
